...
正式版本由 `release-v*` 标签触发 GitHub Actions，并从 [CHANGELOG](CHANGELOG.md) 提取发布说明。macOS 和 HarmonyOS 构建均使用 GitHub-hosted Apple Silicon runner；Release 页面固定提供 `harmony-netbridge.zip`（Apple Silicon Mac 二进制）和 `harmony-netbridge.hap.zip`（release 模式构建、绑定调试设备的 debug Profile 签名 HAP）。签名材料通过受保护的 GitHub Environment Secret 注入，不进入仓库；HAP 仅能安装到 Profile 已登记的设备。首次配置 Environment、签名 Secret 与发版步骤见 [Release workflow](docs/releasing.md)。
